...
export default function DoctorLabRequests() {
  ...
  const handleSubmit = async () => {
    if (!formData.patientId || !formData.testType) { Alert.alert('Error', 'Please select patient and test type'); return; }
    setSubmitting(true);
    try {
    ...
    <View style={styles.card}>
      <View style={styles.row}>
        <View>
          ...
          {item.acceptedBy && <Text style={styles.accepted}>Accepted by: {item.acceptedBy.name} on {formatDate(item.acceptedAt)}</Text>}
        </View>
      ...
      {item.status === 'completed' && (
        <>
          {item.resultFile && <Image source={{ uri: item.resultFile }} style={styles.resultImage} />}
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList data={requests} keyExtractor={(item) => item._id} renderItem={renderItem} ListEmptyComponent={<Text style={styles.empty}>No lab requests.</Text>} />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.selectRow}>
              ...
              <FlatList horizontal data={patients} keyExtractor={(item) => item._id}
                renderItem={({ item }) => (
                  <TouchableOpacity onPress={() => setFormData({ ...formData, patientId: item._id })}
                    style={[styles.patientOption, formData.patientId === item._id && styles.patientOptionActive]}>
                    <Text style={[styles.patientText, formData.patientId === item._id && styles.patientTextActive]}>{item.name}</Text>
                  </TouchableOpacity>
                )}
              />
            </View>
            <TextInput style={styles.input} placeholder="Test Type" value={formData.testType} onChangeText={(text) => setFormData({ ...formData, testType: text })} />
            <TextInput style={[styles.input, styles.textArea]} placeholder="Description (optional)" value={formData.description} onChangeText={(text) => setFormData({ ...formData, description: text })} multiline />
...
const styles = StyleSheet.create({
  ...
  createBtn: { flexDirection: 'row', backgroundColor: '#3b82f6', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 8, alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
